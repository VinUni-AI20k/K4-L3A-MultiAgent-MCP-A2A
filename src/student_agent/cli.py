...
async def _run(root: Path) -> None:
    ...
    for idx, case_id in enumerate(case_set.case_ids):
        target = output_root / f"{case_id}.json"
        if target.exists():
            print(f"[{case_id}] already exists ({idx + 1}/{total}), skipping...")
            continue
        ...
        for attempt in range(max_attempts):
            try:
                async with connect_gateway(
                    ...
                    print(f"[{case_id}] completed ({idx + 1}/{total})")
                    break
            except Exception as exc:
                if attempt == max_attempts - 1:
                    raise
                print(
                    f"[{case_id}] attempt {attempt + 1} failed: {exc}. Retrying in 2s..."
                )
