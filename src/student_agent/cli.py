...
async def _run(root: Path, *, resume: bool = False) -> None:
        ...
        async with semaphore:
            ...
            for attempt in range(1, 7):
                try:
                    ...
                    break
                except Exception as exc:  # A fresh MCP session is the safe retry boundary.
                    last_error = exc
                    if attempt < 6:
                        await asyncio.sleep(min(5 * attempt, 20))
            if output is None:
                raise RuntimeError(f"{case_id} failed after 6 MCP sessions") from last_error
    ...
    pending = [
        ...
        if not resume or not (output_root / f"{case_id}.json").exists()
    ]
    await asyncio.gather(*(process_case(case_id) for case_id in pending))
...
def parser() -> argparse.ArgumentParser:
    ...
    run.add_argument("--resume", action="store_true", help="skip outputs already present")
